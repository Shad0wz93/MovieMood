...
from als_model_wrapper import ALSModelWrapper
...
with mlflow.start_run(run_name=f"ALS_User_{user_id}"):
    ...
    model, ids, scores = processing(matrix_csr, user_index)
    recommended_df, user_ratings_sorted = post_processing(movies, ratings, item_ids, ids, scores, user_id)

    # Log du modèle dans MLflow
    mlflow.pyfunc.log_model(
        artifact_path="als_recommendation_model",
        python_model=ALSModelWrapper(model, user_index, item_ids)
    )
